/**
 * Thin wrapper for @justgoingviral/mcpo
 * Forwards calls to the underlying package at runtime
 */

import { Tool } from '@modelcontextprotocol/sdk/types.js';

// Define tool schemas that match the JustGoingViral server
export const justGoingViralTools: Tool[] = [
  {
    name: "contacts",
...
// Handler for JustGoingViral tools
export async function handleJustGoingViralTool(name: string, args: any) {
  try {
    // @ts-ignore - Dynamic import resolved at runtime
    const { createServer } = await import('@justgoingviral/mcpo');
    const server = createServer();
    return await server.callTool(name, args);
  } catch (error) {
    console.error(`[JustGoingViral Wrapper] Error:`, error);
    return {
      content: [{ type: 'text', text: `JustGoingViral tool ${name} failed: ${error}` }],
      isError: true
    };
  }
}
